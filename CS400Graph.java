...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if (!containsVertex(start))
            throw new NoSuchElementException("No vertex with data " + start);;
        if (!containsVertex(end))
            throw new NoSuchElementException("No vertex with data " + end);

        // a table that corresponds all vertices in this graph to
        // the shortest path found starting from the given
        // start data item to the corresponding vertex data item
        Hashtable<T, Path> pathsTable = new Hashtable<>();

        // a set of all unvisited vertices in this graph
        Set<T> unvisited = new HashSet<>(vertices.keySet());

        // The first item in the table is from start data item to itself
        pathsTable.put(start, new Path(vertices.get(start)));
        while (!unvisited.isEmpty()) {
            // get all the paths in the table of all the vertices
            // that are unvisited into a PriorityQueue
            PriorityQueue<Path> paths = pathsTable.entrySet().stream()
                    .filter(e -> unvisited.contains(e.getKey()))
                    .map(Map.Entry::getValue)
                    .collect(Collectors.toCollection(PriorityQueue::new));

            try {
                // get the minimum path and its end vertex using the PriorityQueue
                Path shortest = paths.remove();
                Vertex current = shortest.end;

                // for each leaving edges of the vertex, extend the minimum
                // path with this edge and add it to the table with the corresponding
                // end vertex of this extended path if the path is smaller than
                // what is already in the table or there is no path that corresponds
                // to this end vertex yet, add it to the table
                for (Edge edge : current.edgesLeaving.stream()
                        .filter(e -> unvisited.contains(e.target.data))
                        .collect(Collectors.toList())) {
                    Path newShortest = new Path(shortest, edge);
                    Path toTarget = pathsTable.get(edge.target.data);
                    if (toTarget == null || toTarget.compareTo(newShortest) > 0)
                        pathsTable.put(edge.target.data, newShortest);
                }

                // remove the current chosen vertex from the set of unvisited vertices
                unvisited.remove(current.data);
            } catch (NoSuchElementException e) {
                // if it reaches here, any vertices still in the unvisited set means
                // they are impossible to reach from the start data item, discard them
                break;
            }
        }

        // get the path in the table that the end data item corresponds to
        // and check its validity
        Path p = pathsTable.remove(end);
        if (p == null || !p.start.data.equals(start) || !p.end.data.equals(end))
